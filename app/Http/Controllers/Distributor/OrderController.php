...
class OrderController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(Auth::user()->account_role == 'distributor') {
            if ($request->has('end_date')) {
                $orders = $orders->where('orders.site_code', $user->site_code)->whereBetween('orders.order_time', [$request->start_date, $request->end_date]);
            }

            if ($request->has('category_id')) {
                $orders = $orders->where('orders.site_code', $user->site_code)->where('category_id', $request->category_id);
            }

            if ($request->has('qty')) {
                $orders = $orders->where('orders.site_code', $user->site_code)->whereBetween('order_detail.qty', [$request->qty, $request->max]);
                // dd($orders);
            }

            if ($request->has('search')) {
                $orders = $orders->where('orders.site_code', $user->site_code)->where('orders.invoice', 'like', '%' . $request->search . '%');
            }
            ...
            $orders = $orders
                        ->select('orders.id', 'orders.order_time', 'orders.invoice', 'orders.name', 'mapping_site.branch_name', 'orders.payment_total', 'orders.payment_final', 'orders.status', 'orders.point', 'orders.status_faktur')
                        ...
                        ->where('orders.site_code', $user->site_code)
                        ->orderBy('orders.id', 'DESC')
                        ->groupBy('order_detail.order_id', 'orders.id', 'mapping_site.id', 'orders.order_time', 'orders.invoice', 'orders.name', 'mapping_site.branch_name', 'orders.payment_total', 'orders.payment_final', 'orders.status', 'orders.point', 'orders.status_faktur')
            ...
            if ($request->has('end_date')) {
                $orders = $orders->whereIn('orders.site_code', $array_child)->whereBetween('orders.order_time', [$request->start_date, $request->end_date]);
            }

            if ($request->has('category_id')) {
                $orders = $orders->whereIn('orders.site_code', $array_child)->where('category_id', $request->category_id);
            }

            if ($request->has('qty')) {
                $orders = $orders->whereIn('orders.site_code', $array_child)->whereBetween('order_detail.qty', [$request->qty, $request->max]);
                // dd($orders);
            }

            if ($request->has('search')) {
                $orders = $orders->whereIn('orders.site_code', $array_child)->where('orders.invoice', 'like', '%' . $request->search . '%');
            }
	...
	public function store(Request $request)
    {
        ...
        $logs->activity     = "Create new order";
    ...
    public function reportSales(Request $request)
	{
        ...
        if ($request->start_date && $request->end_date) {
            ...
            $order  = $order
                    ->with(['data_item.product', 'data_user.user_address', 'order_details.promo.reward'])
                    ->where('status', '2')
                    ->orWhere('status', '3')
                    ->orWhere('status', '4')
                    ->where('status_faktur', $status_faktur);
