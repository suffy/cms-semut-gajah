...
class ComplaintController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $product_complaint = $this->complaints->where('id', $id)->with('product')->first();
    ...
    public function confirm($id)
    {
        ...
        if($complaint->option == "pengembalian dana") {
            ...
            $activity   = "Dana dengan order invoice " . $complaint->order->invoice . " sudah dikembalikan ke kredit";
            ...
            $half                       = NULL;
            if($complaint->half == 1) {
                $konversi_sedang_ke_kecil   = $konversi_sedang_ke_kecil / 2;
                $half                       = 1;
            }
            ...
            if($product->status_promosi_coret) {
                $harga_ritel_gt         = $product->price->harga_ritel_gt * $complaint->qty;
                $harga_grosir_mt        = $product->price->harga_grosir_mt * $complaint->qty;  
            ...
            Http::post('http://site.muliaputramandiri.com/restapi/api/master_data/orders_detail', [
                    ...
                    // 'item_disc'         => $orderDisc, // update 08 - 10 - 21
                    'item_disc'         => $rp_cabang, // update 08 - 10 - 21
                    'total_price'       => $totalPrice, 
                    'notes'             => 'retur complaint',
                    'created_at'        => Carbon::now()->format('Y-m-d H:i:s'),
                    'updated_at'        => Carbon::now()->format('Y-m-d H:i:s'),
