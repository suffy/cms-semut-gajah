...
<section class="panel">
    ...
    <div class="card-body">
               ...
               <form method="get" action="@if(auth()->user()->account_role == 'manager'){{url('manager/report-sales')}}@else{{ url('superadmin/report-sales') }}@endif">
               <div class="row d-flex">
                    ...
                    <div class="col-sm-9 col-md-2 col-lg-1 my-1 align-self-end">
                        <a href="@if(auth()->user()->account_role == 'manager'){{url('manager/report-sales/item/excel')}}@else{{ url('superadmin/report-sales/item/excel') }}@endif" class="btn btn-blue">Export Excel</a>
                    </div>
                </div>
                </form>
                <div class="row d-flex" style="margin-top:10px;">
                    <div class="col-sm-3 col-md-2 col-lg-1 my-1 align-self-end">
                        <a href="@if(auth()->user()->account_role == 'manager'){{url('manager/report-sales/item?status_faktur=F')}}@else{{ url('superadmin/report-sales/item?status_faktur=F') }}@endif" class="btn @if(\Illuminate\Support\Facades\Request::get('status_faktur')=="F") btn-blue @elseif(\Illuminate\Support\Facades\Request::get('status_faktur')=="R") btn-neutral @else btn-blue @endif" data-name="item">Faktur</a>
                    </div>
                    <div class="col-sm-9 col-md-2 col-lg-1 my-1 align-self-end">
                        <a href="@if(auth()->user()->account_role == 'manager'){{url('manager/report-sales/item?status_faktur=R')}}@else{{ url('superadmin/report-sales/item?status_faktur=R') }}@endif"  class="btn @if(\Illuminate\Support\Facades\Request::get('status_faktur')=="F") btn-neutral @elseif(\Illuminate\Support\Facades\Request::get('status_faktur')=="R") btn-blue @else btn-neutral @endif" data-name="transaksi">Retur</a>
                    </div>
                </div>
            </div>
                            ...
                            <div class="row" id="display-report">
                                <table id="table-report" class="table default-table export-table" >
                                    ...
                                    @forelse($orders as $order)
                                            ...
                                            @if($item->product_id)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if($item->promo) 
                                                            @php 
                                                                $reward = $item->whereNull('product_id')->where('order_id', $order->id)->where('promo_id', $item->promo_id)->first(); 
                                                            @endphp
                                                            
                                                            @if($reward->disc_xtra)
                                                                {{$reward->disc_xtra}}%
                                                            @elseif($reward->rp_xtra)
                                                                Rp. {{number_format($reward->rp_xtra,2)}}
                                                            @elseif($reward->point_xtra)
                                                                {{$reward->point_xtra}} Point
                                                            @elseif($reward->bonus)
                                                                {{$reward->bonus_name}} - ({{$reward->qty}})
                                                            @else
                                                            @endif 
                                                        @else
                                                            Tidak Dapat Promo
                                        ...
                                        @endforeach
                                        <tr>
                                            <td colspan="10">Total Per Order</td>
                                            <td style="text-align:center">Rp. {{ number_format($order->payment_final, 2) }}</th>
